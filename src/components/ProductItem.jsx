...
const ProductItem = ({ id, image, name, price, discount }) => {
    ...
    const { getDiscountedPrice } = useContext(ShopContext) || { getDiscountedPrice: (p, d) => d > 0 ? p * (1 - d / 100) : p };
    ...
    return (
        <Link className='group relative bg-white rounded-xl overflow-hidden shadow-sm hover:shadow-2xl transition-all duration-500 border border-gray-100' to={`/product/${id}`}>
            ...
            <div className='relative overflow-hidden bg-gradient-to-br from-pink-50 to-rose-50'>
                ...
                {/* Discount Badge */}
                {discount > 0 && (
                    <div className='absolute top-3 left-3 z-10 bg-red-500 text-white text-[10px] font-black px-2 py-1 rounded-lg shadow-lg uppercase tracking-tighter'>
                        {discount}% OFF
                    </div>
                )}
                ...
                <div className='flex items-center justify-between'>
                    <div>
                        <p className='text-lg font-bold text-pink-500'>
                            {currency}{Math.round(getDiscountedPrice(price, discount))}
                        </p>
                        {discount > 0 && (
